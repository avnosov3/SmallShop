...
from shop.choices import PaymentStatusChoices, OrderStatusChoices
...
class OrderAdmin(BaseAdmin):
    ...
    def handle_order_confirmation(self, order):
        ...
        order.save()
        post_order.delay({'id': order.id, 'total_amount': str(order.total_amount), 'time_accepted': str(order.time_accepted)})
...
class OrderProductAdmin(BaseAdmin):
    ...
    list_filter = ('order',)
    search_fields = ('product__name', 'product__content',)
